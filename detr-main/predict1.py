...
def plot_result(pil_img, prob, boxes, save_name=None, imshow=False, imwrite=True):
    ...
    boxes_array = np.concatenate((zero_column, prob, boxes), axis=1)
    ...
    with open('D:/DetectionAlgorithm/DataSet/DETR/DETRoutput/pretect/1test/label/{}'.format(save_name[:-4]) +'.txt', 'w') as file:
	...
	# 修改成自己要保存的目录
    if imwrite:
        if not os.path.exists("D:/DetectionAlgorithm/DataSet/DETR/DETRoutput/pretect/1test"):  #########
            os.makedirs('D:/DetectionAlgorithm/DataSet/DETR/DETRoutput/pretect/1test')     ###########
        cv2.imwrite('D:/DetectionAlgorithm/DataSet/DETR/DETRoutput/pretect/1test/{}'.format(save_name), opencvImage)  ##########
...
def load_model(model_path, args):
    ...
    model.cuda()
    model.eval()
    state_dict = torch.load("D:/DetectionAlgorithm/DataSet/DETR/DETRoutput/train/checkpoint0499.pth")  # <--------model_path---修改加载模型的路径
...
def detect(im, model, transform, prob_threshold=0.7):
    ...
    EachT = end - start
    return probas[keep], bboxes_scaled, EachT
...
if __name__ == "__main__":
    ...
    # 加载模型 修改成自己路径
    dfdetr = load_model('D:/DetectionAlgorithm/DataSet/DETR/DETRoutput/train/checkpoint0499.pth', main_args)  # <--修改为自己加载模型的路径
    # <--修改为待预测图片所在文件夹路径
    ...
    for file in files:
        ...
        scores, boxes, EachT = detect(im, dfdetr, transform)
