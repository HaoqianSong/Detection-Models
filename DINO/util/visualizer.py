...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, imagename, caption=None, dpi=120, savedir=None, show_in_console=True):
        """
        img: tensor(3, H, W)
        ...
        if savedir is not None:
            if caption is None:
                savename = '{}/{}.jpg'.format(savedir, imagename[:-4])#int(tgt['image_id']), str(datetime.datetime.now()).replace(':', '-')
            else:
                savename = '{}/{}-{}.png'.format(savedir, caption, imagename)#int(tgt['image_id']), str(datetime.datetime.now()).replace(' ', '-')
