...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, imagename, caption=None, dpi=300, savedir=None, show_in_console=True):
        """
        img: tensor(3, H, W)
        ...
        if savedir is not None:
            if caption is None:
                savename = '{}/{}.png'.format(savedir, imagename[:-4]) #int(tgt['image_id']), str(datetime.datetime.now()).replace(' ', '-')
            else:
